...
{
    public partial class ReleaseDetianLicense : Form
    {
        public static int CurrentLicenseID { get; set; }
        public static int AppID { get; set; }
        public ReleaseDetianLicense()
        {
            InitializeComponent();
        }

        private void ReleaseDetianLicense_Load(object sender, EventArgs e)
        {
                ...
                if (CurrentLicenseID != 0)
                {
                    ...
                    if (DLicense != null)
                    {
                        if (!DLicense.IsReleased)
                            LoadDrivingLicenseInfo();
                        else MessageBox.Show($"The License with ID = {DLicense.LicenseID} is Already Released!");
                    }
                    else MessageBox.Show($"There is No Detained License With ID = {CurrentLicenseID}");
                }
        ...
        private void LoadDrivingLicenseInfo()
        {
            ...
            if (Llicense != null)
            {
                ...
                //{
                ucReleaceDetianInfo.OldAppID = ucDriverLicenseInfo.AppID;
                ucReleaceDetianInfo.OldLicenseID = CurrentLicenseID;
                ucReleaceDetianInfo.RaiseOldAppIdChanged();
        ...
        private void AddReleaseDetainedLicenseApp()
        {
            clsLicensesBL Licenses1 = clsLicensesBL.FindLicenseByLicenseID(CurrentLicenseID);
            clsApplicationsBL App1 = clsApplicationsBL.FindApplicationByApplicationID(Licenses1.ApplicationID);
            if (Licenses1 != null)
            {
                if (App1 != null)
                {
                    ...
                    app2.ApplicationTypeID = 5;
                    app2.ApplicationStatus = 3;
                    app2.LastStatusDate = DateTime.Today;
                    app2.ApplicationFees = clsApplicationTypesBL.FindApplicationTypeByID(5).Fees;
                    ...
                    if (app2.SaveApp())
                    {
                        ...
                        if (DLicense != null)
                        {
                            int UserID = clsGlobalSettings.User.UserID;
                            if (clsDetainedLicensesBL.ReleaseDetainedLicense(DLicense.DetainID, DateTime.Today, UserID, app2.ApplicationID))
                            {
                                MessageBox.Show($"The License With ID = {DLicense.LicenseID} " +
                                    $"has Been Released Successfully!");
                                clsLicensesBL.DeactivateLicense(CurrentLicenseID);
                                btnRelease.Enabled = false;
                                ucReleaceDetianInfo.NewAppID = app2.ApplicationID;
                                ucReleaceDetianInfo.NewLicenseID = CurrentLicenseID;
                                ucReleaceDetianInfo.RaiseOldAppIdChanged();
                                LlblShowLicenseInfo.Enabled = true;

                            }
                            else MessageBox.Show($"The License With ID = {DLicense.LicenseID} " +
                                    $"has NOT Been Released!");

                        }
                    }
                    else MessageBox.Show("The Released Application has NOT Issued!");
                }
            }
        }
